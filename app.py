...
import pandas as pd
from ipyleaflet import Map, GeoData, basemaps, FullScreenControl, GeoJSON, Popup, Heatmap
from shinywidgets import output_widget, render_widget
import faicons as fa
import json
import geopandas as gpd
from data_preparation import DP, gpt_transformation, address_transformation, column_transformation, column_reorganization, data_visualization
import webbrowser
from pandas import json_normalize
import feature_engineering as fe
import plotly.express as px
import numpy as np
from model_training import fit, benchmark_tpot, benchmark_h2o, benchmark_flaml, benchmark_pycaret, benchmark_mjar
    ...
    ui.nav_panel("Data Collection",
                 ui.page_sidebar(
                     ui.sidebar(
                         ...
                         ui.input_action_link('survey_link', 'User Feedback Survey'),
                     ...
                     ui.sidebar(
                         ui.accordion(
                             ui.accordion_panel("Step 1: Data Upload",
                                                ui.input_text("osm_feature_folder_path",
                 ...
                 ui.page_sidebar(
                     ui.sidebar(
                         ...
                         ui.HTML("""
                                    ...
                                    [1] <b>PFI plot</b>: A Permutation Feature Importance (PFI) plot ranks features by their impact 
                                    on model performance (higher bars = more important features), 
                                    showing which features most degrade predictions when shuffled. <br>
                                    
                                    [2] <b>SHAP summary plot</b>: The SHAP summary plot shows feature importance (vertical order, higher bars = more important features), 
                                    revealing how each feature influences model predictions positively or negatively across samples. <br>
                                    
                                    [3] <b>PDP plot</b>: A Partial Dependence Plot (PDP) shows how a feature influences model predictions 
                                    on average, visualizing the average effect of the values of a particular feature by 
                                    marginalizing all other features in the feature set. <br>
                                    
                                    [4] <b>SHAP dependence plot</b>: The SHAP dependence plot reveals how a single feature's values 
                                    impact model predictions (vertical SHAP values) while uncovering potential interactions with another feature (color-coded). <br>
                                    
                                    [5] <b>SHAP values</b>: SHAP values quantify the contribution of each feature to a model's prediction for a specific instance, 
                                    showing both magnitude (importance) and direction (positive/negative impact).
                                    </p>
                                    """
                                    ),
                         width=500, open='always'
...
def server(input, output, session):
    ...
    @reactive.effect
    @reactive.event(input.survey_link)
    def _():
        webbrowser.open('https://docs.google.com/forms/d/e/1FAIpQLSfL41IeEpeK76e9h3NNK8n7SaZy1nSCQCDH8dLzkCszowKc1g/viewform?usp=dialog', new=2)
    ...
    def clean_data():
        ...
        if file is None:
            ...
            # start data preparation process
                ...
                data = data_processor.data_clean()

                # if Step 3 has information
                if len(input.ordinal()) != 0:
                    for id, column in enumerate(input.ordinal()):
                        data = column_transformation(data,
                                                     rankings=eval('input.ranking_results{}()'.format(str(id))),
                                                     columns=input.ordinal())
                # if Step 4 has information
                if len(input.reorganize()) != 0:
                    for id, column in enumerate(input.reorganize()):
                        data = column_reorganization(data,
                                                     code=eval('input.codes{}()'.format(str(id))),
                                                     column=column)
                        data = data_processor.reorganization_clean(data, column)
    ...
    def _():
        if input.feature_boundary() is not None:
            point_feature_list = fe.osm_point_features(
                osm_path=input.osm_feature_folder_path(),
                boundary=fe.boundary_import(boundary_file=input.feature_boundary()[0]["datapath"]))['fclass'].unique()

            line_feature_list = fe.osm_line_features(
                osm_path=input.osm_feature_folder_path(),
                boundary=fe.boundary_import(boundary_file=input.feature_boundary()[0]["datapath"]))['fclass'].unique()

            polygon_feature_list = fe.osm_polygon_features(
                osm_path=input.osm_feature_folder_path(),
                boundary=fe.boundary_import(boundary_file=input.feature_boundary()[0]["datapath"]))[1][
                'fclass'].unique()

            ui.update_selectize(
                'point_features',
                choices=point_feature_list.tolist())

            ui.update_selectize(
                'line_features',
                choices=line_feature_list.tolist())

            ui.update_selectize(
                'polygon_features',
                choices=polygon_feature_list.tolist())
    ...
    def feature_table():
        if input.clean_data() is not None:
            ...
            # run the main func
            fe.stream_feature_generation(data=pd.read_csv(input.clean_data()[0]["datapath"]),
                                                        folder=input.osm_feature_folder_path(),
                                                        boundary=fe.boundary_import(
    ...
    def statistics():
        if input.clean_data() is not None:
            return fe.statistics(preparation_feature_data().iloc[:, 1:])
            ...
            if input.automl() is False:
                ...
                if input.automl_choices() == 'tpot':
                    if input.advanced() is True:
                        tpot_results = benchmark_tpot(data=data,
                                                      target=input.target_col(),
                                                      train_size=input.train_test(),
                ...
                if input.automl_choices() == 'h2o':
                    if input.advanced() is True:
                        h2o_results = benchmark_h2o(data=data,
                                                    target=input.target_col(),
                                                    train_size=input.train_test(),
                ...
                if input.automl_choices() == 'flaml':
                    if input.advanced() is True:
                        flaml_results = benchmark_flaml(data=data,
                                                    target=input.target_col(),
                                                    train_size=input.train_test(),
                ...
                if input.automl_choices() == 'pycaret':
                    if input.advanced() is True:
                        pycaret_results = benchmark_pycaret(data=data,
                                                        target=input.target_col(),
                                                        train_size=input.train_test(),
                ...
                if input.automl_choices() == 'mjar':
                    if input.advanced() is True:
                       mjar_results = benchmark_mjar(data=data,
                                                     target=input.target_col(),
                                                     train_size=input.train_test(),
